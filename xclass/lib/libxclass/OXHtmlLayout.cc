...
OHtmlLayoutContext::OHtmlLayoutContext() {
  pStart = 0;
  pEnd = 0;
  leftMargin = 0;
  rightMargin = 0;
...
void OHtmlLayoutContext::PushMargin(SHtmlMargin **ppMargin,
...
void OHtmlLayoutContext::PopOneMargin(SHtmlMargin **ppMargin) {
  if (*ppMargin) {
...
void OHtmlLayoutContext::PopMargin(SHtmlMargin **ppMargin, int tag) {
  int bot = -1;
  int oldTag;
  SHtmlMargin *pM;
...
void OHtmlLayoutContext::PopExpiredMargins(SHtmlMargin **ppMarginStack,
                                           int y) {
...
void OHtmlLayoutContext::ClearMarginStack(SHtmlMargin **ppMargin) {
...
OHtmlElement *OHtmlLayoutContext::GetLine(OHtmlElement *p_start,
                OHtmlElement *p_end, int width, int minX, int *actualWidth) {
  int x;                        // Current X coordinate
  int spaceWanted = 0;          // Add this much space before next token
  OHtmlElement *p;              // For looping over tokens
  OHtmlElement *lastBreak = 0;  // Last line-break opportunity
...
//           HtmlTokenName(p), p->input.x, p->input.w));
      ...
      case Html_DD: {
        ...
        if (((OHtmlListStart *)ref->pOther)->compact == 0 ||
            x + spaceWanted >= 0) {
          *actualWidth = ((x <= 0) && !isEmpty) ? 1 : x;
          return p;
        }
...
void OHtmlLayoutContext::FixAnchors(OHtmlElement *p, OHtmlElement *p_end,
                                    int y) {
  while (p && p != p_end) {
    if (p->type == Html_A) ((OHtmlAnchor *)p)->y = y;
    p = p->pNext;
  }
}
...
int OHtmlLayoutContext::FixLine(OHtmlElement *p_start,
               OHtmlElement *p_end, int bottom, int width,
...
void OHtmlLayoutContext::Paragraph(OHtmlElement *p) {
  int headroom;
  ...
  if (p->type == Html_Text) {
    ...
    OHtmlTextElement *text = (OHtmlTextElement *) p->pNext;
    headroom = text->ascent + text->descent;
  } else {
    //// headroom = 10;
...
void OHtmlLayoutContext::ComputeMargins(int *pX, int *pY, int *pW) {
  int x, y, w;
...
void OHtmlLayoutContext::ClearObstacle(int mode) {
...
// Return the next markup type  [OHtmlElement::NextMarkupType]

int OXHtml::NextMarkupType(OHtmlElement *p) {
  while (p = p->pNext)
...
OHtmlElement *OHtmlLayoutContext::DoBreakMarkup(OHtmlElement *p) {
  ...
  switch (p->type) {
    ...
    case Html_DIR:
    case Html_OL:
      if (((OHtmlListStart *)p)->compact == 0) Paragraph(p);
    ...
    case Html_EndDIR: {
      ...
      if (ref->pOther) {
        ...
        if (!((OHtmlListStart *)ref->pOther)->compact) Paragraph(p);
      }
      break;
    }
    ...
    case Html_TABLE:
      pNext = TableLayout((OHtmlTable *) p);
      break;
...
int OHtmlLayoutContext::InWrapAround() {
...
void OHtmlLayoutContext::WidenLine(int reqWidth, int *pX, int *pY, int *pW) {
...
void OHtmlLayoutContext::LayoutBlock() {
...
void OHtmlLayoutContext::PushIndent() {
...
void OHtmlLayoutContext::PopIndent() {
...
void OXHtml::LayoutDoc() {
  ...
  layoutContext.pageWidth = _canvas->GetWidth();
  layoutContext.left = 0;
#else
  layoutContext.headRoom = margins.t;
  layoutContext.pageWidth = _canvas->GetWidth() - margins.r;
  layoutContext.left = margins.l;
#endif
