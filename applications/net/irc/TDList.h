 ...
 *
 *
 * Requires classes to have valid copy construct and assignment operator.
 * --------
 *
 ...
 * += item Calls Add(item)
 * AddAfterWork(item) adds to the Works next pointer and moves the Work pointer
 *			to item.
 ...
 *    The node will only clean it's self.
 *
 */
  protected:
    typedef struct Node {
        ...
        Node *Next;
        Node *Prev;
        Node(T item) : Next(0), Prev(0), Data(item) {}
        ~Node() {}
    } Node;
...
/*
 *    The node will only clean it's self and the data memory.
 *
 */
  protected:
    typedef struct Node {
