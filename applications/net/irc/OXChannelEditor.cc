...
OXChannelEditor::OXChannelEditor(const OXWindow *p,const OXWindow *main,
  ...
  OXTransientFrame(p, main, 10, 10) {
    ...
    f2->SetLayoutManager(new O2ColumnsLayout(f2, 5, 5));
    AddFrame(f2,_l4);
...
void OXChannelEditor::_SaveInfo(){
  ...
  if(_nameTE->GetTextLength()>0){
	_ci->name = new char[_nameTE->GetTextLength()+1];
	strcpy(_ci->name,_nameTE->GetString());
  } else _ci->name =0;

  if(_ci->logfile) delete _ci->logfile;
  if(_logfileTE->GetTextLength()>0){
	_ci->logfile = new char[_logfileTE->GetTextLength()+1];
	strcpy(_ci->logfile,_logfileTE->GetString());
  } else _ci->logfile =0;

  if(_ci->background) delete _ci->background;
  if(_backTE->GetTextLength()>0){
	_ci->background = new char[_backTE->GetTextLength()+1];
	strcpy(_ci->background,_backTE->GetString());
